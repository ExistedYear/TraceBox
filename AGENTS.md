...
- **Trace Intelligence diagnostics**: Gemini/provider and intelligence-route failures emit structured server logs with the operation, model, request ID, provider status/message, and stable `AI_*` code; logs redact API-key-like values and never include prompts or raw model output. Gemini `v1beta` structured JSON requests use flat `responseMimeType`/`responseJsonSchema` generation fields for model compatibility, and HTTP provider status takes precedence over message text when mapping errors.
