...
- **DB types are generated**: edit schema via migration, then `npm run db:types` or `npm run db:types:linked`; do not hand-edit `src/types/database.ts`. The committed linked contract and hosted ledger are reconciled through migration 085. Nullable RPC arguments that use database defaults are passed as `undefined` at typed call sites.
...
- Committed Playwright browser harness lives under `playwright/`; run `npm run test:e2e:list` for discovery or `npm run test:e2e` for public/auth smoke. Authenticated journeys use environment-gated disposable fixtures when enabled; GitHub route/webhook coverage stays credential-free, and browser reports, state, and credentials remain ignored.
