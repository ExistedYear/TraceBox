...
This is the authoritative, source-level completion tracker for TraceBox. It consolidates the two source audits formerly represented by `docs/incomplete.md` and `docs/incomplete_v2.md`; each gap appears once and has a stable ID. A capability is complete only when a user can discover, configure, execute, recover from failure, and observe it through an authorized product workflow.
...
- Status: EXTERNAL
- Evidence: migrations `202608260002` and `202608260043`; `/dashboard/settings/members`; `/dashboard/settings/contributors`; `/invite/[token]`; `src/components/settings/{workspace-members-manager,project-members-manager,invitation-acceptance}.tsx`; persistent sidebar and issue-queue Contributors links; `tests/membership-migration.test.ts`
...
- Status: OPEN
- Evidence: `src/components/issues/issue-table.tsx`; `src/lib/issues.ts`; roadmap queue requirements around `docs/tracebox-main-plan.md:1618-1644`
...
- Status: OPEN
- Evidence: `src/components/triage/triage-inbox.tsx`; triage requirements around `docs/tracebox-main-plan.md:1671-1692`
...
- Status: IN PROGRESS
- Evidence: migrations `202608260040`–`202608260043`; reconciled `src/types/database.ts`; synchronized `supabase/full_schema.sql`; stale GitHub/API casts removed. Static checks pass, but this environment could not run Docker-backed migration replay or `npm run db:types`.
...
- Status: OPEN
- Evidence: `docs/tracebox-main-plan.md`; migrations `001`, `017`, `018`, `028`, `030`; `supabase/seed.sql`
...
- Status: DONE
- Evidence: `docs/plan.md` describes the completed deployment foundation and points to the Phase 20 roadmap and deployment checklist.
- Acceptance: `docs/plan.md` clearly labels the foundation plan as historical/completed and does not imply the whole product or hosted validation is complete.
