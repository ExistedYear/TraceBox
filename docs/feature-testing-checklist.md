# TraceBox Feature Reference

This is the submission feature reference for TraceBox. It describes the supported product behaviors by area so reviewers can navigate the implementation. GitHub App and email-provider behaviors require the corresponding optional service configuration; the core tracker does not depend on either integration.
...
- First user creates a workspace and first project; the project receives the default workflow and starts with an empty, configurable component catalog.
...
- Disposable Supabase reset, pgTAP authorization suite, and true concurrent allocator test pass in CI/Docker.
- `npm run lint`, `npm run typecheck`, `npm test`, `npm run build`, and `npm run check:migrations` pass.
