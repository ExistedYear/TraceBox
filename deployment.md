...
### 1.3 Apply Database Migrations (1 through 24) via SQL Script

You do **not** need the Supabase CLI. You can apply all 24 migrations directly in the Supabase web dashboard:

#### Method A: Single Consolidated Script (Recommended)

1. Open the Supabase Dashboard → click **SQL Editor** in the left sidebar.
2. Click **+ New Query**.
3. Open the file `supabase/full_schema.sql` from this repository (which consolidates all 24 ordered migrations).
...
3. Confirm that the following 6 tables are checked:
   - `comments`
   - `issues`
   - `notifications`
   - `issue_watchers`
...
Once deployed (or running locally), verify the full user experience:
...
4. **Issue Creation**: Click **New issue** (`/dashboard/issues/new`), fill in Title, Description, Severity, and Priority, and click **Create issue**. Verify it redirects to `CORE-1`.
...
6. **Issue Detail & Collaboration**:
   ...
   - Link related issues (e.g. `BLOCKS`, `DUPLICATE_OF`).
7. **Notification Center**: Trigger an event or mention and check the bell icon in the top header.
8. **Settings**: Go to `/dashboard/settings` and test component creation, label management, version archival, and workflow visualization.
9. **Log Out**: Click your avatar menu in the top right → **Log out**. Verify protected routes redirect to `/login`.
