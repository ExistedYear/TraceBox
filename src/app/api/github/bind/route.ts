...
export async function POST(request: NextRequest) {
  ...
  const { error } = await supabase.rpc("bind_github_repository", {
...
export async function DELETE(request: NextRequest) {
  ...
  const body = await readBody(request);
  if (!body) return NextResponse.json({ error: "Invalid GitHub repository binding." }, { status: 400 });
  const { error } = await supabase.rpc("unbind_github_repository", { p_project_id: body.projectId, p_github_repository_id: body.repositoryId });
  if (error) {
