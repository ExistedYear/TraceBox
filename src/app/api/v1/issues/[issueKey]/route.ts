...
import { authenticateApiRequest, filterApiVisibleIssues, getApiMutationErrorStatus, type ApiScope } from "@/lib/api-auth";
...
async function getIssue(request: NextRequest, issueKey: string, scope: ApiScope) {
  ...
  const { data: project, error: projectError } = await auth.client.from("projects").select("id").eq("key", parsed.projectKey).eq("organization_id", auth.context.organizationId).eq("is_archived", false).maybeSingle();
  if (projectError) return { auth: null, response: NextResponse.json({ error: "Could not load the project." }, { status: 500 }) } as const;
  ...
  if (error) return { auth: null, response: NextResponse.json({ error: "Could not load the issue." }, { status: 500 }) } as const;
  if (!issue) return { auth: null, response: NextResponse.json({ error: "Issue not found." }, { status: 404 }) } as const;
  let visibleIds: string[];
  try {
    visibleIds = await filterApiVisibleIssues(auth.client, auth.context, [issue]);
  } catch {
    return { auth: null, response: NextResponse.json({ error: "Could not verify issue access." }, { status: 500 }) } as const;
  }
...
export async function GET(request: NextRequest, { params }: { params: Params }) {
  ...
  if (error) return NextResponse.json({ error: "Could not load the issue." }, { status: 500 });
...
export async function PATCH(request: NextRequest, { params }: { params: Params }) {
  ...
  if (error) {
    console.error("API issue update failed", { code: error.code, message: error.message, issueId: result.issue.id });
    return NextResponse.json({ error: "Could not update issue." }, { status: getApiMutationErrorStatus(error) });
  }
