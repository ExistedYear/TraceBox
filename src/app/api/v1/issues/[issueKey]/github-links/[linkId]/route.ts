...
export async function DELETE(request: NextRequest, { params }: { params: Params }) {
  ...
  const { data: link, error: linkError } = await db.from("issue_github_links").select("id").eq("id", linkId).eq("issue_id", result.issue.id).maybeSingle();
  if (linkError) return NextResponse.json({ error: "Could not load the GitHub link." }, { status: 500 });
  ...
  if (error) {
    ...
    return NextResponse.json({ error: "Could not remove GitHub link." }, { status: getApiMutationErrorStatus(error) });
  }
