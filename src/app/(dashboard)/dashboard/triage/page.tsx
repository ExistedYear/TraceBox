...
export default async function TriagePage() {
      ...
      <main className="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">
        <Surface className="p-12 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-xs text-muted-foreground">
            {canCreateProject ? "Select or create a project to triage incoming issues." : "Select a project to triage incoming issues."}
          </p>
  ...
  return (
    <TriageInbox
      key={`${projectId}-${formattedIssues.map((issue) => `${issue.id}:${issue.updatedAt}`).join("|")}`}
      projectId={projectId}
