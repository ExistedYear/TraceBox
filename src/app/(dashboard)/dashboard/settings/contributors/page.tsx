...
export default async function ContributorsPage() {
  ...
  if (!context.activeProject) return <main className="mx-auto max-w-[1100px] p-4 sm:p-6 lg:p-8"><EmptyState icon={FolderKanban} title="No project selected" description="Choose a project from the sidebar to manage contributors." /></main>;
  ...
  if (loadError) {
    ...
    return <main className="mx-auto max-w-[1100px] p-4 sm:p-6 lg:p-8"><Surface className="p-8 text-center"><h1 className="text-lg font-semibold">Contributors unavailable</h1><p className="mt-2 text-sm text-muted-foreground">We could not load project access. Try again in a moment.</p><Button asChild variant="outline" className="mt-5"><Link href="/dashboard/settings/contributors">Retry</Link></Button></Surface></main>;
  }
