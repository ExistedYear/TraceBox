...
export default async function SettingsPage() {
  ...
  const [{ data: project }, { data: components }, { data: states }, { data: transitions }, { data: memberRows }, { data: profileRows }, { data: canManage }] =
    await Promise.all([
  ...
  const displayNameById = new Map((profileRows ?? []).map((row) => [row.id, row.display_name]));
