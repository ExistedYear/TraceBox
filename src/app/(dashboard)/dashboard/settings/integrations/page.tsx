...
export default async function IntegrationsSettingsPage() {
  ...
  const [{ data: legacy }, { data: installations, error: installationsError }, { data: bindings, error: bindingsError }, { data: operations, error: operationsError }] = await Promise.all([
  ...
  const { data: organizationRepositories, error: repositoriesError } = installationIds.length
    ...
    : { data: [], error: null };
  const loadError = installationsError ?? bindingsError ?? repositoriesError ?? operationsError;
  if (loadError) {
