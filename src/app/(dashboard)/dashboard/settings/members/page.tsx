...
import { WorkspaceVisibility } from "@/components/settings/workspace-visibility";
...
export default async function WorkspaceMembersPage() {
  ...
  const [{ data: rows, error: membersError }, { data: canManage, error: manageError }, { data: organization, error: organizationError }] = await Promise.all([
    ...
    supabase.from("organizations").select("is_public").eq("id", context.activeOrganization.id).maybeSingle(),
  ]);
  ...
  const loadError = membersError ?? manageError ?? organizationError ?? invitationResult.error;
  if (loadError) {
  ...
  return <div className="mx-auto max-w-[1100px] p-4 sm:p-6 lg:p-8"><div className="mb-8"><p className="mb-2 font-mono text-xs font-semibold uppercase tracking-[0.18em] text-primary">{context.activeOrganization.name} · Access</p><h1 className="text-3xl font-semibold tracking-tight">Workspace members</h1><p className="mt-2 text-sm text-muted-foreground">Invite collaborators, manage workspace roles, transfer ownership, or leave safely.</p></div>{canManage ? <><WorkspaceVisibility organizationId={context.activeOrganization.id} initialPublic={Boolean(organization?.is_public)} /><WorkspaceMembersManager organizationId={context.activeOrganization.id} currentUserId={context.userId} members={members} invitations={invitations} canManage /></> : <><Surface className="mb-6 p-6"><p className="text-sm text-muted-foreground">You can view workspace membership, but only workspace administrators can manage other people&apos;s access.</p></Surface><WorkspaceMembersManager organizationId={context.activeOrganization.id} currentUserId={context.userId} members={members} invitations={[]} canManage={false} /></>}</div>;
}
