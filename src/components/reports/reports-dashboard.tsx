...
export function ReportsDashboard({ projectName, projectKey, projectId, initialMetrics }: Props) {
  ...
  return <main className="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">
    ...
    {!error && !metrics.no_data ? <>
      ...
      <Surface className="mt-5 p-4"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-sm font-semibold">Historical trend</h2><p className="text-xs text-muted-foreground">Daily created, resolved, and point-in-time backlog</p></div><span className="font-mono text-[11px] text-muted-foreground">{metrics.historical_trend.length} days</span></div><div className="space-y-2">{metrics.historical_trend.slice(-30).map((point) => <div key={point.day} className="grid grid-cols-[70px_1fr_1fr_1fr] items-center gap-2 text-[11px]"><span className="font-mono text-muted-foreground">{point.day}</span><span className="h-2 rounded bg-primary/70" style={{ width: `${Math.min(100, point.created * 12 + (point.created ? 4 : 0))}%` }} title={`Created ${point.created}`} /><span className="h-2 rounded bg-emerald-500/70" style={{ width: `${Math.min(100, point.resolved * 12 + (point.resolved ? 4 : 0))}%` }} title={`Resolved ${point.resolved}`} /><span className="h-2 rounded bg-amber-500/70" style={{ width: `${Math.min(100, point.backlog * 4 + (point.backlog ? 4 : 0))}%` }} title={`Backlog ${point.backlog}`} /></div>)}</div><div className="mt-3 flex gap-4 text-[11px] text-muted-foreground"><span>Created</span><span>Resolved</span><span>Backlog</span></div></Surface>
