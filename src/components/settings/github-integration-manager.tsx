...
export function GithubIntegrationManager({ projectId, canManage, initialLegacyRepo, initialInstallations, initialRepositories, initialBindings, initialWebhookDeliveries }: Props) {
  ...
  async function updateBinding(binding: Binding) {
    ...
    try {
      const response = await fetch("/api/github/bind", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: projectId, github_repository_id: binding.github_repository_id, is_primary: binding.is_primary, auto_resolve_enabled: draft?.autoResolveEnabled ?? binding.auto_resolve_enabled, target_branches: branches }) });
