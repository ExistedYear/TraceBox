...
import { ExternalLink, Github, Link2, RefreshCw, Trash2 } from "lucide-react";
...
export function GithubIntegrationManager({ projectId, initialLegacyRepo, initialInstallations, initialRepositories, initialBindings }: Props) {
  ...
  async function bindRepository() {
    if (busy || !selectedRepositoryId) return;
    ...
    setBusy(true);
    try {
      const response = await fetch("/api/github/bind", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: projectId, github_repository_id: selectedRepositoryId, is_primary: true, auto_resolve_enabled: autoResolve, target_branches: branches }) });
  ...
  async function unbindRepository(repositoryId: string) {
    if (busy || !window.confirm("Disconnect this GitHub repository from the project? Historical links will remain.")) return;
    setBusy(true);
    try {
  ...
  return <section className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-center gap-2"><Github className="h-4 w-4 text-foreground" /><div><h2 className="text-sm font-semibold">GitHub App</h2><p className="text-xs text-muted-foreground">Connect verified repositories without granting TraceBox access to your GitHub password.</p></div></div><div className="flex flex-wrap gap-2"><Button asChild size="sm" className="h-8 gap-1 text-xs"><a href={`/api/github/connect?project_id=${encodeURIComponent(projectId)}`}><Github className="h-3.5 w-3.5" /> Connect GitHub</a></Button><Button type="button" variant="outline" size="sm" className="h-8 gap-1 text-xs" onClick={() => void refreshRepositories()} disabled={busy || !installations.length}><RefreshCw className={`h-3.5 w-3.5 ${busy ? "animate-spin" : ""}`} /> Refresh</Button></div></div>
    ...
    {installations.length > 0 && <div className="space-y-3 rounded-[10px] border border-border/70 p-4"><div><h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Project repositories</h3><p className="mt-1 text-[11px] text-muted-foreground">A project can use multiple repositories. The primary repository receives compatibility webhook matching.</p></div><div className="grid gap-2 md:grid-cols-[1fr_auto]"><label className="sr-only" htmlFor="github-repository-picker">GitHub repository</label><select id="github-repository-picker" className="h-8 min-w-0 rounded-md border border-input bg-background px-2 text-xs" value={selectedRepositoryId} onChange={(event) => selectRepository(event.target.value)}><option value="">Choose an accessible repository</option>{availableRepositories.map((repository) => <option key={repository.id} value={repository.id}>{repository.full_name}{repository.private ? " · private" : ""}</option>)}</select><Button type="button" size="sm" className="h-8 gap-1 text-xs" onClick={() => void bindRepository()} disabled={busy || !selectedRepositoryId}><Link2 className="h-3.5 w-3.5" /> Connect repository</Button></div><div className="grid gap-3 sm:grid-cols-[1fr_1fr]"><label className="space-y-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Target branches<input className="mt-1 h-8 w-full rounded-md border border-input bg-background px-2 text-xs font-normal normal-case tracking-normal text-foreground" value={targetBranches} onChange={(event) => setTargetBranches(event.target.value)} placeholder="main, release/*" /></label><label className="flex items-center gap-2 self-end pb-1 text-xs text-muted-foreground"><input type="checkbox" checked={autoResolve} onChange={(event) => setAutoResolve(event.target.checked)} /> Resolve `Fixes KEY-123` only on these branches</label></div><ul className="divide-y divide-border/60 rounded border border-border/70">{bindings.length === 0 ? <li className="p-3 text-xs text-muted-foreground">No repositories connected to this project.</li> : bindings.map((binding) => { const repository = repositories.find((item) => item.id === binding.github_repository_id); return <li key={binding.github_repository_id} className="flex flex-wrap items-center justify-between gap-3 px-3 py-2 text-xs"><span className="min-w-0"><span className="font-medium">{repository?.full_name ?? "Unavailable repository"}</span>{binding.is_primary && <span className="ml-2 rounded bg-primary/10 px-1.5 py-0.5 font-mono text-[10px] text-primary">primary</span>}<span className="ml-2 text-[10px] text-muted-foreground">{binding.target_branches.join(", ")}</span></span><span className="flex items-center gap-2"><a href={repository?.html_url} target="_blank" rel="noreferrer" className="text-primary hover:underline" aria-label={`Open ${repository?.full_name ?? "repository"}`}><ExternalLink className="h-3 w-3" /></a><Button type="button" variant="ghost" size="sm" className="h-6 px-1.5 text-muted-foreground hover:text-destructive" onClick={() => void unbindRepository(binding.github_repository_id)} disabled={busy} aria-label={`Disconnect ${repository?.full_name ?? "repository"}`}><Trash2 className="h-3 w-3" /></Button></span></li>; })}</ul></div>}
