...
export function AppSidebar({ organizations, projects, activeOrganizationId, activeProjectId }: ShellNavProps) {
  ...
  return <aside className={cn("hidden shrink-0 border-r border-border/80 bg-card/35 transition-[width] duration-200 md:block", collapsed ? "w-[72px]" : "w-64")}><SidebarContent collapsed={collapsed} onCollapse={() => setCollapsed((value) => !value)} switcher={switcher} /></aside>;
}
