...
export type ShellNavProps = { organizations: WorkspaceSummary[]; projects: ProjectSummary[]; activeOrganizationId: string; activeProjectId: string | null; canCreateIssue: boolean };
...
function SidebarContent({ mobile = false, collapsed = false, onCollapse, onNavigate, switcher, canCreateIssue }: { mobile?: boolean; collapsed?: boolean; onCollapse?: () => void; onNavigate?: () => void; switcher?: React.ReactNode; canCreateIssue: boolean }) {
  return <div className={cn("flex h-full min-h-0 min-w-0 flex-col overflow-x-hidden overflow-y-auto", mobile ? "p-4" : collapsed ? "p-2" : "p-3")}>
    ...
    {canCreateIssue ? <div className={cn("mt-5", collapsed && "mt-8")}>
...
export function AppSidebar({ organizations, projects, activeOrganizationId, activeProjectId, canCreateIssue }: ShellNavProps) {
  ...
  return <aside className={cn("sticky top-0 hidden h-screen shrink-0 border-r border-border/80 bg-card/35 transition-[width] duration-200 md:block", collapsed ? "w-[72px]" : "w-64")}><SidebarContent collapsed={collapsed} onCollapse={() => setCollapsed((value) => !value)} switcher={switcher} canCreateIssue={canCreateIssue} /></aside>;
}

export function MobileSidebar({ organizations, projects, activeOrganizationId, activeProjectId, canCreateIssue }: ShellNavProps) {
