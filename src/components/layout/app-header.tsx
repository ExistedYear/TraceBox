...
type AppHeaderProps = { email: string; displayName?: string | null; avatarUrl?: string | null; workspaceName?: string | null; projectName?: string | null; organizations: WorkspaceSummary[]; projects: ProjectSummary[]; activeOrganizationId: string; activeProjectId: string | null };
...
export function AppHeader({ email, displayName, avatarUrl, workspaceName, projectName, organizations, projects, activeOrganizationId, activeProjectId }: AppHeaderProps) {
  ...
  return <><header className="sticky top-0 z-30 flex min-h-14 items-center justify-between gap-3 border-b border-border/80 bg-background/90 px-3 backdrop-blur sm:px-5"><div className="flex min-w-0 items-center gap-2 sm:gap-3"><MobileSidebar organizations={organizations} projects={projects} activeOrganizationId={activeOrganizationId} activeProjectId={activeProjectId} /><div className="hidden min-w-0 items-center gap-2 text-xs text-muted-foreground sm:flex"><span className="font-mono text-[11px] text-primary">TB</span>{workspaceName && <><span>/</span><span className="max-w-44 truncate">{workspaceName}</span></>}{projectName && <><span>/</span><span className="max-w-44 truncate">{projectName}</span></>}<span>/</span><span className="shrink-0">{getPageName(pathname)}</span></div><button className="hidden h-8 min-w-48 items-center gap-2 rounded-md border border-border/80 bg-card/60 px-2.5 text-left text-xs text-muted-foreground hover:bg-accent md:flex" onClick={() => setPaletteOpen(true)}><Search className="h-3.5 w-3.5" /><span className="flex-1">Search workspace</span><kbd className="rounded border bg-muted px-1.5 py-0.5 font-mono text-[10px]">⌘ K</kbd></button><button className="flex h-8 items-center gap-2 rounded-md px-2 text-xs text-muted-foreground hover:bg-accent md:hidden" onClick={() => setPaletteOpen(true)} aria-label="Open search"><Search className="h-4 w-4" /></button></div><div className="flex items-center gap-0.5"><ThemeToggle /><UserMenu email={email} displayName={displayName} avatarUrl={avatarUrl} /></div></header>{paletteOpen && <CommandPalette onClose={() => setPaletteOpen(false)} />}</>;
}
