...
export async function replayGithubWebhookDeliveries(admin: any, limit = 25) {
  ...
  const [{ data: pending, error: pendingError }, { data: stale, error: staleError }] = await Promise.all([
  ...
  if (pendingError || staleError) {
    throw new Error(`GITHUB_WEBHOOK_REPLAY_LOOKUP_FAILED:${pendingError?.code ?? staleError?.code ?? "unknown"}`);
  }
