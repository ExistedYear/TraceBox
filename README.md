...
`supabase/migrations/` holds 72 ordered migrations. Migrations 042–064 cover GitHub reliability, completion-plan closure, analytics, collaboration, and operational visibility. Migrations 065–072 are forward-only live-schema reconciliations for API scopes, RPC grants, advisor findings, runtime SQL ambiguity, indexes/RLS planning, and volatility. Regenerate `supabase/full_schema.sql` after every migration change. Because Supabase does not re-run an applied version when its file changes, compare the linked ledger and live catalog before every push and repair drift with a new migration.
