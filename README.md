...
3. Copy `.env.example` to `.env.local` and add the URL and anon key from Supabase. For the API and GitHub webhook routes, also add `SUPABASE_SERVICE_ROLE_KEY` and `GITHUB_WEBHOOK_SECRET`; both remain server-only.
...
`supabase/migrations/` holds thirty-nine ordered migrations through Phase 20 and the release-validation corrections. These cover the foundational schema/RLS/RPC hardening, collaboration/planning, notifications/realtime, search/saved views, issue links, triage, attachments/storage policies, templates, restricted issue access, GitHub integrations/webhooks, typed custom fields, granular API scopes, and final audit corrections. The consolidated `supabase/full_schema.sql` contains the same 39 migrations in order; run `npm run sync:migrations` after adding a migration.
...
Push this repository to GitHub and connect it to Vercel with the Next.js preset. Configure `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` for browser/server use. Configure `SUPABASE_SERVICE_ROLE_KEY` only as a non-public Vercel server environment variable because authenticated REST API routes and the GitHub webhook use it server-side. Configure `GITHUB_WEBHOOK_SECRET` for webhook signature verification. See `deployment.md` for the full setup and verification sequence.

Apply all migrations before testing production signup. Configure Supabase Auth URLs, the private `issue-attachments` Storage bucket/policies, the `supabase_realtime` publication, the GitHub webhook, and Vercel server environment variables before the live end-to-end flow.
